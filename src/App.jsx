...
// ENS 解析:name -> address(走 Cloudflare 公共节点)
  ...
  const fetchTrxData = async (q, pageNum) => {
    ...
    if (nD.data && Array.isArray(nD.data)) {
      ...
      txs = txs.concat(nD.data.filter(tx => tx.amount && parseFloat(tx.amount) > 0).map(tx => ({
        ...
        amount: formatAmount(tx.amount, 6), symbol: 'TRX', type: 'NATIVE',
    ...
    if (list && Array.isArray(list)) {
      ...
      txs = txs.concat(list.map(tx => {
        ...
        return {
          ...
          amount: formatAmount(tx.quant, dec), symbol: sym, type: 'TOKEN',
            ...
            {loading ? <ListSkeleton />
              ...
              : paginated.map((tx, i) => (
                <TxRow key={`${tx.hash}-${i}`} tx={tx} currentAddress={currentQuery.address}
                  remarkMap={remarkMap} onAddressClick={jumpToAddress} />
              ))}
          </div>
...
// ============ 交易行 ============
function TxRow({ tx, currentAddress, remarkMap, onAddressClick }) {
